...
export const AuthProvider = ({ children }) => {
    ...
    const [statusLogin, setStatusLogin] = useState(StatusLoginValues.INITIALIZING);
    ...
    useEffect(()=> {
        console.log(`[${statusLogin}] inProgress:${inProgress}`);
        if (inProgress !== InteractionStatus.None) {
            console.log( "msal in progress. Wait...");
            return;
        }      // still busy

        // console.log( `===> useEffect called for status: ${statusLogin}. accounts=`, accounts);
        // Wait for MSAL to be idle before proceeding with auth logic
        if (statusLogin === StatusLoginValues.INITIALIZING) {
            console.log("MSAL initialization finished. Moving to check login status.");
            setTimeout(()=> {
                if (statusLogin === StatusLoginValues.INITIALIZING) {
                    setStatusLogin(StatusLoginValues.UNDEFINED);
                }
            }, 100);
            return; // Wait for re-render to process UNDEFINED state
        }

        const isLoggedIn = loggedUsing=== LoggedUsingValues.AZURE_AD?
        ...
        // }
        console.log('isLoggedIn:', isLoggedIn, 'accounts', accounts, "instance", instance);
        ...
        switch(statusLogin) {
                    ...
                    do_register2()
                        .then((email) => {
                            ...
                            if (email) {
                                ...
                                console.log("[register] need to wait more. Will be triggered with changes in instance or accounts");
                            }
                        })
                        .catch(reason => {
            ...
            case StatusLoginValues.REGISTER_FAILED: //=> login screen
                break;
            default:
                break; // Added default case for safety
        }
    }, [statusLogin, accounts, instance, inProgress, accessTokenForEmail])

    // persist actions
    useEffect(() => {
        if (loggedUsing !== null) { // Avoid storing initial null value
    ...
    // persist actions
    useEffect(() => {
        if (accessTokenForEmail !== null) { // Avoid storing initial null value
