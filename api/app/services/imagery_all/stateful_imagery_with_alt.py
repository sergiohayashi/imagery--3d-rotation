...
class StatefulImageryModuleWithAlt:
    ...
    def close(self):
        for plotter in self.plotter_map.values():
            ...
            try:
                plotter.close()  # releases the VTK/EGL context
                plotter.deep_clean()
            except Exception:
                pass
    ...
    def rotate(self, target: str, mode="yaw", step_size=360 / 12):
    ...
    def show(self, target):
        # self.plotter.show()
        self.plotter_map[target].show(auto_close=False, jupyter_backend="static")

    def save_image(self, target, output_path: str = None):
        # Save screenshot
        # filename = f"output_images/rotate_{mode}_{i:03d}.png"
        self.plotter_map[target].screenshot(output_path)
        print(f"Saved {output_path}")

    def rotate_full(self, target, mode, step, prefix):
        step_size = 360 / step
        files = []
        for i in range(step):
            self.rotate(mode, step_size)
            filename = f"output_images/{prefix}_{mode}_{i:03d}.png"
            self.plotter_map[target].screenshot(filename)
    ...
    def run_sequence_and_save_image(
        ...
        plt.suptitle(f"rotation target: {target}", fontsize=18, color="black", y=1.04)
