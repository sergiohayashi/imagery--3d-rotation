...
* there is always one (among the alternatives), and only one, correct answer.
...
* The examples of command sequences are only **examples**. You don't need to follow these examples. If you understand how to use the commands, use in full power to help anchor your reasoning. Variability in which command use, the degree, the sequence length, the number of iterations, the targets. Use all the possibilities to conduct your investigation and help you get the more precise and correct answer as possible.
* The rotation commands should be always relative to the last state of the object. If you generate the 'up' command, it will show the bottom view, relative to the last generate image. Remember that it is a continuation. So, make sense to say, "I want to see the hidden part of the object on the top" for example. In this case you should generate the "down:15" command.
* for any suspicion on your partial conclusion, say "wait" and back to more detailed and double check investigation. Imagery way of think is move around the objects and check though multiple angles, repeating the same angle. It is like repeating the steps hypothesize, verify (though images), hypothesize, verify, bolding the confidence.
* what, in usual reasoning problem can be redundunt, in imagery way of thinking it is not. It is just double checking because visual clue can be missed or misinterpreted easily. Take this into account.
...
class ToolsBackedImageryReasoner_Eval_08103_p3:
