...
prompt_for_reasoner = """
The problem consists of a static 2D image.
...
The imagery module has structural knowledge of the object in the problem. It can manipulate the object consistently and return the result of the manipulation as a static 2D image (think of it as a camera view of the object).

To interact with the imagery module, you generate as your output the image manipulation (camera movement) commands (details below). The imagery module will receive this command, execute it, generate the resulting snapshot images, and it will be passed back to you in the next iteration.

So, this is an iterative process.

You may repeat the process as many times as you want.

The imagery module is stateful. the current state of the object is the result of the most recent manipulation you requested.
...
All reasoning and the final solution are entirely your responsibility. The imagery module only assists by providing alternative views and serving as an anchor for your reasoning process.

In you turn, at the end of each iteration, output the following information:
- current_iteration: the current iteration number. The iteration number is a sequential, starting with 1.
- rationale: your reasoning so far (this is your memory)
- next_step: what to investigate in the next iteration
- commands: the command(s) for the imagery module (details below)
- final_answer: your answer to the visual problem or null if you are not sure yet. Providing a value for this field will end the process. Otherwise set as null.

Everything you output will be provided as context to the next iteration. However, for images returned by the imagery module, only the last 3 images will be provided (image memory is shorter). Therefore, enrich the rationale in your output.

# Commands

You may generate commands for more than one target, and for each target you may provide a sequence of commands.
...
- Yaw is a rotation around global Z (think longitude around the object).
...
```
{
  "thought_process": {
    ...
    "rationale": "your reasoning so far; this is your memory",
    "next_step": "what you will investigate in the next iteration"
  },
  "commands": [
    {
...
- Initially, do not focus on the final result. Instead, focus on analyzing and deepening your understanding of each alternative and the original object. 
- Execute at least 5 iterations. From iteration 6 onwards, you may continue analysis or generate the final result.
...
class ToolsBackedImageryReasoner_Eval92:
