...
- **Goal:** Proof.
- **Action:** Compare `original` and `Suspect`.
  ...
  "commands": [
    {
      ...
      "rotation_sequence": "right:15,up:15,left:15"
    }
  ],
  "final_answer": null 
}
...
About the fields:
- `rotation_sequence`: You can ask for a sequence of commands for one target. Each step will produce a snapshot thumbnail like image.
- `target`: You can ask for more than a target. Each target will produce one image.
- `final_answer`: set the final answer once you have proof. This will end the iteration process. Or, leave as null.

---

## 5. Imagery Module Capabilities
- **Stateful:** Models retain orientation. 
- **Context:** You see the last 3 turns.
- **Commands:** Valid rotation operations are: `left:<degree>`, `right:<degree>`, `up:<degree>`, `down:<degree>`, `rotate:cw:<degree>`, `rotate:ccw:<degree>`.
- left,right,up,down: rotate the model to the given direction, by the given degree.
- rotate:cw,rotate:ccw: rotate the model locally, clockwise or counter-clockwise, by the given degree.
...
class ToolsBackedImageryReasoner_Eval_05009:
