...
- **Example:** `right:90,right:90,up:45,right:45` (Target: "A")
...
- **Goal:** Proof.
- **Action:** Compare `original` and `Suspect` in the same turn.
...
```json
{
  ...
  "commands": [
    {
      "target": "B", 
      "rotation_sequence": "left:90,up:-20,left:90"
    }
  ],
  "final_answer": null 
}
...
- **Commands:** `left/right` (orbit), `up/down` (vertical), `rotate:cw/ccw` (spin view).
- Rotations are performed in camera space (around the camera's own axes), NOT in world/object space (around a fixed "up" axis like global Z).
...
3.  Have I done the side-by-side comparison?
...
class ToolsBackedImageryReasoner_Eval_08104_p10:
