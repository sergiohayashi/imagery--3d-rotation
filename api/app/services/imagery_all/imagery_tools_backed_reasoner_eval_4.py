...
- remember, they are blocks of size 1, stacked. 
- The entire block can have different height when you rotate (change the camera position). So, don't rely on height, unless tied to axis.
- the question alternatives are different views, that are result of the rotation (any axis) of the original model.
- use features that don't change with rotation, like the number of boxes.
- the resulting color or shades can differ depending on the rendering. Don't depend on that.
- use more spatial information of the model, the relative position between the boxes. This don't change!
...
**  IMPORTANT **
The imagery model never change the model itself. Just give a different perspecive (camera position) of the same underlyning 3D model structure.
...
# always show grid to provide axis information
plotter.show_grid()   
...
** IMPORTANT **
- Stick to simple perspective change like camera position ONLY. 
- You can't modify the underlyning 3D model, just do litle camera changes to give the user a different perspective (of the same model).
- Generate always a clear view of the underlyning object.
...
class ToolsBackedImageryReasoner_Eval4:
    ...
    async def reason_loop(
        ...
        while iter_count <= MAX_ITERATION:
            ...
            reasoner_system_message = (
                ...
                else reasoner_with_answer
            )

            for _ in range(3):  # max 3 times, in case the output is not json
