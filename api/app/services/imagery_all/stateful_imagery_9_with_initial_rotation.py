...
class StatefulImageryWithInitialRotationModule_9:
    ...
    def rotate_human(self, target: str, action: str, value=None):
        ...
        Supported:
          ...
          - cw:<deg>  -> roll clockwise
          - ccw:<deg> -> roll counter-clockwise
        ...
        if action in {"cw", "ccw"}:
            if value is None:
                raise ValueError(
                    "cw or ccw requires a direction and degrees: cw:30 or ccw:30"
                )
            deg = float(value)
            signed = -deg if action == "cw" else +deg
        ...
        raise ValueError(
            f"Unknown human action '{action}'. Valid: left/right/up/down/cw/ccw/view"
        )
    ...
    def run_human_sequence_and_save_image(
        ...
        Example:
          "view:y, up:30, up:30, left:45, cw:20"
        ...
        action_sequence = action_sequence.replace("rotate:cw", "cw").replace(
            "rotate:ccw", "ccw"
        )
        ...
        for i, cmd in enumerate(cmds):
            ...
            # if action == "rotate":
            #     if len(parts) != 3:
            #         raise ValueError(f"Invalid rotate command '{cmd}'. Use rotate:cw:<deg> or rotate:ccw:<deg>")
            #     direction = parts[1].lower()
            #     deg = float(parts[2])
            #     human_value = f"{direction}:{deg}"
            #     self.rotate_human(target, "rotate", f"{direction}:{deg}")
            #     label_cmd = f"rotate:{direction}:{int(deg) if deg.is_integer() else deg}"
