...
Assume the camera-space rotation, which means, is relative to the current camera angle.
`rotate:cw` rotates clockwise; `rotate:ccw` rotates counterclockwise.
...
Object state can be in different angles from the problem description. Use angle 0 (ex: `left:0`) to get the current state snapshot image.
...
class ToolsBackedImageryReasoner_Eval_07099_Prompt2:
