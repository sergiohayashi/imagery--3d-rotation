...
def register(subparsers):
    parser = subparsers.add_parser("eval08101_prompt1_v2")
    parser.add_argument("--start", required=False)
    parser.add_argument("--end", required=False)
    parser.add_argument("--list", required=False)
    parser.set_defaults(handler=EvalRunner08101Prompt1_v2)


class EvalRunner08101Prompt1_v2(CommandBase):
    ...
    async def run_eval(
        ...
        chat_message: ChatMessage = ChatMessage(
            ...
            use_model=[
                ModelWithParameters(name="tools-based-imagery--eval-08101_prompt1_v2")
            ],
            preset_list=[
                {
                    "role": "user",
