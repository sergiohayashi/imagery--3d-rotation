...
def register(subparsers):
    parser = subparsers.add_parser("eval6")
    parser.add_argument("--start", required=False)
    parser.add_argument("--end", required=False)
    parser.set_defaults(handler=EvalRunner6)


class EvalRunner6(CommandBase):
    ...
    async def run_eval(
        ...
        chat_message: ChatMessage = ChatMessage(
            ...
            use_model=[ModelWithParameters(name="tools-based-imagery--eval-6")],
            preset_list=[
                {
                    "role": "user",
                    "content": None,
