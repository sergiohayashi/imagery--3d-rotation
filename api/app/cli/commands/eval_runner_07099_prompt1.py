...
def register(subparsers):
    parser = subparsers.add_parser("eval07099_prompt1")
    parser.add_argument("--start", required=False)
    parser.add_argument("--end", required=False)
    parser.add_argument("--list", required=False)
    parser.set_defaults(handler=EvalRunner07099Prompt1)


class EvalRunner07099Prompt1(CommandBase):
    ...
    async def __call__(self):
        ...
        for idx, problem in enumerate(problems_to_eval):
            ...
            with open(result_dir / f"result.json", "w") as f:
                json.dump({"idx": idx, "result": result}, f, indent=2)
            with open(result_dir / f"score.json", "w") as f:
                json.dump({"idx": idx, "score": score / len(result)}, f)
    ...
    async def run_eval(
        ...
        chat_message: ChatMessage = ChatMessage(
            ...
            use_model=[
                ModelWithParameters(name="tools-based-imagery--eval-07099_prompt1")
            ],
            preset_list=[
                {
                    "role": "user",
