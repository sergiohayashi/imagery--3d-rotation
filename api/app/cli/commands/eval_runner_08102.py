...
def register(subparsers):
    parser = subparsers.add_parser("eval08102")
    parser.add_argument("--start", required=False)
    parser.add_argument("--end", required=False)
    parser.add_argument("--list", required=False)
    parser.set_defaults(handler=EvalRunner08102)


class EvalRunner08102(CommandBase):
    ...
    async def __call__(self):
        ...
        # read the bounds
        with open(
            Path(self.root_path, "foundation_bounds--with-alt-and-rotation.json")
        ) as f:
    ...
    async def run_eval(
        ...
        chat_message: ChatMessage = ChatMessage(
            ...
            use_model=[ModelWithParameters(name="tools-based-imagery--eval-08102")],
            preset_list=[
                {
                    "role": "user",
                    "content": None,
