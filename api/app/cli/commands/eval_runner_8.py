...
def register(subparsers):
    parser = subparsers.add_parser("eval8")
    parser.add_argument("--start", required=False)
    parser.add_argument("--end", required=False)
    parser.add_argument("--list", required=False)
    parser.set_defaults(handler=EvalRunner8)


class EvalRunner8(CommandBase):
    ...
    async def run_eval(
        ...
        chat_message: ChatMessage = ChatMessage(
            ...
            use_model=[ModelWithParameters(name="tools-based-imagery--eval-8")],
            preset_list=[
                {
                    "role": "user",
                    "content": None,
