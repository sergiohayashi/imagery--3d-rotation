...
def register(subparsers):
    parser = subparsers.add_parser("eval95")
    parser.add_argument("--start", required=False)
    parser.add_argument("--end", required=False)
    parser.add_argument("--list", required=False)
    parser.set_defaults(handler=EvalRunner95)


class EvalRunner95(CommandBase):
    ...
    async def run_eval(
        ...
        chat_message: ChatMessage = ChatMessage(
            ...
            use_model=[ModelWithParameters(name="tools-based-imagery--eval-95")],
            preset_list=[
                {
                    "role": "user",
                    "content": None,
