...
def register(subparsers):
    parser = subparsers.add_parser("eval7")
    parser.add_argument("--start", required=False)
    parser.add_argument("--end", required=False)
    parser.add_argument("--list", required=False)
    parser.set_defaults(handler=EvalRunner7)


class EvalRunner7(CommandBase):
    ...
    async def __call__(self):
        ...
        for idx, problem in enumerate(problems_to_eval):
            ...
            try:
                ...
                logging.exception(f"Error in EvalRunner7.run_eval: {e}")
                result.append(
                    {
                        "problem_id": problem_id,
                        "title": title,
    ...
    async def run_eval(
        ...
        chat_message: ChatMessage = ChatMessage(
            ...
            use_model=[ModelWithParameters(name="tools-based-imagery--eval-7")],
            preset_list=[
                {
                    "role": "user",
                    "content": None,
