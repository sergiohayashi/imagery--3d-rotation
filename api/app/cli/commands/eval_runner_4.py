...
def register(subparsers):
    parser = subparsers.add_parser("eval4")
    parser.add_argument("--start", required=False)
    parser.add_argument("--end", required=False)
    parser.set_defaults(handler=EvalRunner4)


class EvalRunner4(CommandBase):
    ...
    async def __call__(self):
        ...
        for idx, problem in enumerate(problems[start:end]):
            ...
            answer = await self.run_eval(
                title, problem, image_code, foundation_model_image_url, image_url
            )
    ...
    async def run_eval(
        self, title, problem, image_code, foundation_model_image_url, image_url
    ):
        ...
        chat_message: ChatMessage = ChatMessage(
            ...
            use_model=[ModelWithParameters(name="tools-based-imagery--eval-4")],
            preset_list=[
                {
                    "role": "user",
                    "content": None,
                    ...
                    "foundation_model_code": image_code,
