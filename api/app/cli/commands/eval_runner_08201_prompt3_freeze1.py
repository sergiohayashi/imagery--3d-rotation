...
def register(subparsers):
    parser = subparsers.add_parser("eval08201_p3_f1")
    parser.add_argument("--start", required=False)
    parser.add_argument("--end", required=False)
    parser.add_argument("--list", required=False)
    parser.add_argument("--reasoning-model", required=False)
    parser.set_defaults(handler=EvalRunner08201Prompt3Freeze1)


class EvalRunner08201Prompt3Freeze1(CommandBase):

    root_path = CommandBase.spatialviz_root_path

    def __init__(self, args):
        super().__init__()
        self.args = args
        self.runner = ChatMessageServiceWithImageryForEval2()
    ...
    async def __call__(self):
        ...
        for idx, problem in enumerate(problems_to_eval):
            ...
            # answer = 'A'
            if self.args.reasoning_model:
                the_global_config.cli.reasoning_model = self.args.reasoning_model
            try:
    ...
    async def run_eval(
        self, title, problem, foundation_model_image_url, image_url, problem_bouds_map
    ):
        ...
        chat_message: ChatMessage = ChatMessage(
            ...
            use_model=[
                ModelWithParameters(
                    name="tools-based-imagery--eval-08201_prompt3_freeze1"
                )
            ],
            preset_list=[
                {
