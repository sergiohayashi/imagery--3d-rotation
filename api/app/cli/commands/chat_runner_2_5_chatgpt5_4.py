...
def register(subparsers):
    parser = subparsers.add_parser("chat_gpt_5_4")
    parser.add_argument("--start", required=False)
    parser.add_argument("--end", required=False)
    parser.add_argument("--list", required=False)
    parser.set_defaults(handler=ChatRunner_GPT_5_4)


class ChatRunner_GPT_5_4(CommandBase):
        ...
        for idx, problem in enumerate(problems_to_eval):
            ...
            try:
                chat_message: ChatMessage = ChatMessage(
                    ...
                    use_model=[ModelWithParameters(name="gpt-5.4-2026-03-05")],
                    preset_list=[
                        {
                            "role": "user",
                            "content": None,
