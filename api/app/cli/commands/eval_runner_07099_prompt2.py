...
def register(subparsers):
    parser = subparsers.add_parser("eval07099_prompt2")
    parser.add_argument("--start", required=False)
    parser.add_argument("--end", required=False)
    parser.add_argument("--list", required=False)
    parser.set_defaults(handler=EvalRunner07099Prompt2)


class EvalRunner07099Prompt2(CommandBase):
    ...
    async def run_eval(
        ...
        chat_message: ChatMessage = ChatMessage(
            ...
            use_model=[
                ModelWithParameters(name="tools-based-imagery--eval-07099_prompt2")
            ],
            preset_list=[
                {
                    "role": "user",
