...
def register(subparsers):
    parser = subparsers.add_parser("match101")
    parser.add_argument("--start", required=False)
    parser.add_argument("--end", required=False)
    parser.add_argument("--list", required=False)
    parser.set_defaults(handler=EvalRunnerMatchByRotation00101)


class EvalRunnerMatchByRotation00101(CommandBase):
    ...
    async def run_eval(
        ...
        chat_message: ChatMessage = ChatMessage(
            ...
            use_model=[
                ModelWithParameters(name="tools-based-imagery--match-by-rotation-00101")
            ],
            # preset_list=[
            #     {
            #         "role": "user",
